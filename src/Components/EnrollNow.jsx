...
const photoAPIs = {
  businessfair:
    "https://script.google.com/macros/s/AKfycbxQiXHJs-XxEFLyYq3orTYTsGbyAI-a9LBfz07vWliX03hTPzb3H9J3GzvmsAjUEtwT/exec",
  sparkstudio:
    "https://script.google.com/macros/s/AKfycbyGQ-gT_EsMroe3nOk5GaxZtqExZPHQ86ulOc1MG24iZUCICuDMOLp80KZ7c2jGvVSU/exec",
  ecoquiz:
    "https://script.google.com/macros/s/AKfycbzd-kIiLsfkDxpcXsG4yPDbSTdd-ppgVG4PDQ_d__kfXuYmapCg87t5m_SHjXiRUf9v/exec",
...
export default function ParticipantForm() {
  ...
  const guardarArchivo = (e) => {
    ...
    reader.onload = function (e) {
      ...
      fetch(
        photoAPIs[eventname], //your AppsScript URL
  ...
  const fetchTakenStalls = async () => {
    try {
      ...
      const foodStalls = data.filter(
        (entry) => entry.stallType === "food" || "service" || "games" || "other"
      ); // Filter for food stalls
  ...
  const performOCR = (file) => {
    Tesseract.recognize(
      ...
      .then(({ data: { text } }) => {
       ...
       if (!existingTransactionIds.includes(upiTransactionId)){
        if (
          upiTransactionId ) {
  ...
  const handleSubmit = async () => {
    if (!groupName.trim() && eventLimits[eventname] > 1) {
    ...
    if (eventname === "businessfair") {
      ...
      if (takenStallDetails.includes(stallDetails)) {
        toast.error(
          "This stall detail is already taken. Please choose another."
        );
        return;
      }
    }
    ...
    const data = {
      ...
      ...participants.reduce((acc, participant, index) => {
        ...
        acc[`participant${participantIndex}_collegeName`] =
  ...
  return (
    <div className="participant-form container mt-5">
      {!(["ppt", "seminar6", "seminar1","mmf","mystry","seminar2","ecoquiz","businessfair","sparkstudio","elocution",].includes(eventname)) ? 
      (
        <>
            ...
            <ul>
              {takenStallDetails.map((stall, index) =>
                stall.stallType === stallType ? (
                  <li key={index}>{stall.stallDetails}</li>
                ) : null
              )}
            </ul>
          </>
        ...
        eventname === "seminar6" ? (
          ...
          <>
            ...
            <Form.Group>
              ...
              <Form.Control type="file" onChange={guardarArchivo} />
            </Form.Group>
